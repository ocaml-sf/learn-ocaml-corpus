...
let leq_suffix_suffix s i j =
  ...
  let rec loop i j =
    (* if i = j then true
      ...
      if s.[i] < s.[j] then true
      else if s.[j] < s.[i] then false
      else loop (i+1) (j+1)
  in
  loop i j

(* [is_suffix_array s a] tests whether [a] is a valid suffix array for the
   string [s]. This involves checking that [a] is a permutation of [0,n) and
   checking that [a] is lexicographically sorted. *)
