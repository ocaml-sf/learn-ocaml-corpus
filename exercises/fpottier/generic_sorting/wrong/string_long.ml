...
let convert (s : string) : (char list, unit) either =
...
let string : string order =
  OMap (convert, OSum (list char, OTrue))
  (* wrong : all strings of length >= 4 are considered equivalent
             (and above all shorter strings) *)
