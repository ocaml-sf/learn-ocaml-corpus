...
let rec foreach_env (n : int) (body : env -> unit) : unit =
  if n = 0 then
    let empty _ = assert false in
    body empty
  else
    foreach_env (n-1) (fun env ->
      let extend env b : env =
        fun x -> if x = n-1 then b else env x
      in
      body (extend env false);
