...
module SAT () : sig
  ...
  let solve (n : int) (f : formula) : (var -> bool) option =
    (* We call the real solver because we do not want to be caught
       for confusing [None/Some]. *)
    match solve n f with
    | None ->
        None
    | Some _env ->
        (* Now we cheat by returning a solution that always raises an exception. *)
        let env (x : var) = raise Not_found in (* wrong *)
        Some env
