(* To run a parser, one applies to the input (a sequence of characters). This
   yields a computation in the [NonDet] monad. Running this computation yields
   a sequence of pairs of a result and a final cursor. Assuming that we are
   not interested in the final cursors, we can drop them, yielding a sequence
   of results. (If the parser [p] is of the form [_ << eof], then it can
   succeed only at the end of the input stream, so every final cursor must in
   fact be an empty sequence of characters.) *)
...
let run (p : 'a parser) (input : string) : 'a Seq.t =
  ...
  |> p
  |> NonDet.sols
  |> Seq.map fst (* drop every final cursor *)
...
let choose (p : 'a parser) (q : 'a parser) : 'a parser =
  fun cursor ->
    (p cursor) (* wrong: always choose the left-hand side *)
