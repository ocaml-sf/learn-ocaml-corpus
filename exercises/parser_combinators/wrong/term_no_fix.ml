...
(* In this solution, we use an explicit mutually recursive definition
   instead of using [fix]. This is correct; it works just fine. *)

(* We introduce a fault, which is to accept too much input: we also
   accept reversed parentheses! *)

let rec term cursor =
  chainl1 factor additive_op cursor

and factor cursor =
  chainl1 atom multiplicative_op cursor
