...
let rec first (p : 'a -> bool) (xs : 'a Seq.t) : 'a =
  match xs() with
  | Nil ->
      raise Not_found
  | Cons (x, xs) ->
      if p x then x else first p xs
...
let bmove alpha beta t : move option =
  ...
  | TLeaf _ ->
      None
  | TNonLeaf offspring ->
      (* functionally correct, but too costly:
         compute the value [v] first, then walk
         the list again, looking for a subtree
         of value [v]. *)
      let v = bval alpha beta t in
      Some (fst (first (fun (_, u) ->
        let w = - bval (-beta) (-alpha) u in
        equivalent alpha beta v w
      ) offspring))
