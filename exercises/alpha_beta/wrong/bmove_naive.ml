...
let rec bmove_offspring alpha (candidate : move option) offspring : move option =
  match offspring() with
  | Nil ->
      assert (candidate <> None);
      candidate
  | Cons ((move, t), offspring) ->
      let v = - (bval bottom (* wrong: should be -beta *) (-alpha) t) in
      (* wrong: no cut *)
        let alpha, candidate =
          if alpha < v then
            ...
            alpha, candidate
        in
        bmove_offspring alpha candidate offspring
...
let bmove alpha beta t : move option =
  ...
  | TLeaf v ->
      None
  | TNonLeaf offspring ->
      bmove_offspring alpha None offspring
